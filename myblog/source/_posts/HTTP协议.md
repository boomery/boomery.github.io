---
title: 你可能不知道的HTTP协议
date: 2021-09-23 17:12:35
categories: 技术
---
- [一.应用层协议是什么](#一应用层协议是什么)        
       - [应用协议都有哪些](#应用协议都有哪些)
- [二.在我上网的过程中，都发生了什么](#二在我上网的过程中都发生了什么)         
    - [1.打开浏览器，输入网址URL，看看都有什么](#1打开浏览器输入网址url看看都有什么)         
    -  [2.通过访问的域名找出其IP地址(专门保存了 Web 服务器域名与 IP 的对应关系，它就是 DNS 服务器)](#2通过访问的域名找出其ip地址专门保存了-web-服务器域名与-ip-的对应关系它就是-dns-服务器)         
    -   [3.浏览器发出读取文件(URL 中域名后面部分对应的文件)的HTTP 请求，该请求报文作为 TCP 三次握手的第三个报文的数据发送给服务器](#3浏览器发出读取文件url-中域名后面部分对应的文件的http-请求该请求报文作为-tcp-三次握手的第三个报文的数据发送给服务器)         
    -    [4.服务器给浏览器响应一个301永久重定向响应](#4服务器给浏览器响应一个301永久重定向响应)         
    -    [5.浏览器跟踪重定向地址](#5浏览器跟踪重定向地址)         
    -    [6.服务器处理请求](#6服务器处理请求)          
    -    [7.服务器对浏览器请求作出响应，并把对应的 html 文本发送给浏览器](#7服务器对浏览器请求作出响应并把对应的-html-文本发送给浏览器)          
    -    [8.浏览器显示 HTML](#9浏览器显示-html)         
    -    [9.浏览器发送请求获取嵌入在 HTML 中的资源（如图片、音频、视频、CSS、JS等等）](#10浏览器发送请求获取嵌入在-html-中的资源如图片音频视频cssjs等等) 
- [三.问题来了](#三问题来了)
- [四.原来如此](#四原来如此)
- [五.HTTP1.0  *&*  HTTP1.1  *&* HTTP2.0](#五http10----http11---http20) 
   - [1.HTTP1.0和HTTP1.1的区别](#1http10和http11的区别)      
   - [2.HTTP1.1和HTTP2.0的区别](#2http11和http20的区别)
- [六.HTTPS](#六https) 
- [思考题](#思考题https能被劫持吗)
# 一.应用层协议是什么
...
![a](https://img-blog.csdnimg.cn/img_convert/4f424da9bc5cb5e2dd3e1a3045371b14.png)
---

### 2.通过访问的域名找出其IP地址(专门保存了 Web 服务器域名与 IP 的对应关系，它就是 DNS 服务器)
![b](https://img-blog.csdnimg.cn/img_convert/5f0e5a2415109fc43424b25a681f5659.png)
---

### 3.浏览器发出读取文件(URL 中域名后面部分对应的文件)的HTTP 请求，该请求报文作为 TCP 三次握手的第三个报文的数据发送给服务器
   - 请求长这样
      ![c](https://pic4.zhimg.com/80/v2-5fc6a6e065a3bedb50b31afe4397188b_1440w.jpg)
   ...
   - 接下来应用程序(浏览器)通过调用 Socket 库，来委托协议栈工作。协议栈的上半部分有两块，分别是负责收发数据的 TCP 和 UDP 协议，它们两会接受应用层的委托执行收发数据的操作。
      ![d](https://img-blog.csdnimg.cn/img_convert/0a2d96f21e593c4ef7c6898b90da16ca.png)
   
   - 在 HTTP 传输数据之前，首先需要 TCP 建立连接，TCP 连接的建立，通常称为三次握手。这个所谓的「连接」，只是双方计算机里维护一个状态机，在连接建立的过程中，双方的状态变化时序图就像这样。
   - ![e](https://img-blog.csdnimg.cn/img_convert/69f8c521704b9be654dbb14d2ea3a3fa.png)
...
服务端收到 ACK 的 ACK 之后，处于 ESTABLISHED 状态，因为它也一发一收了。**
   ...
   - 至此，网络包的报文如下图。
   - ![f](https://img-blog.csdnimg.cn/img_convert/5938a55976892eab428048a89a78c979.png)
   - IP 报文生成。
   - ![g](https://img-blog.csdnimg.cn/img_convert/a4ea6f35d2c78f1ea1223cea68abe047.png)
   - MAC 报文生成。
   - ![h](https://img-blog.csdnimg.cn/img_convert/10d87c6a05ab113638f6bc3cec3f34bf.png)
   - 最后网络包格式
   - ![i](https://img-blog.csdnimg.cn/img_convert/0fad4263a847908a44f92c1ca1e35e8c.png)
   - 网卡驱动从 IP 模块获取到包之后，会将其复制到网卡内的缓存区中，接着会其开头加上报头和起始帧分界符，在末尾加上用于检测错误的帧校验序列。
   - ![j](https://img-blog.csdnimg.cn/img_convert/a8fc587e097a697d2d1765e4a7dc0599.png)
   - 互相扒皮 —— 服务器 与 客户端
   - ![k](https://img-blog.csdnimg.cn/img_convert/bfec594f22465eaabbc7b44bd098e17d.png)
---
...
Web服务器解析请求，定位请求资源。服务器将资源复本写到TCP套接字，由客户端读取。一个响应由状态行、响应头部、空行和响应数据4部分组成。
   ...
   - 响应头(Response Header)：由关键字/值对组成，每行一对，关键字和值用英文冒号":"分隔。示例：![l](https://pic4.zhimg.com/80/v2-a373437b39843593dd44ed0dc3255d0f_1440w.jpg)
...
![3.png](https://i.loli.net/2021/05/25/P96XzRIcr24hCMj.png)
...
![s](https://pic4.zhimg.com/80/v2-299b4a71f9b005fa15fbcd4cabfd841b_1440w.png)
所以 HTTP 传输面临的风险有：
